...
def process_war_file():
    with open('D:\military\military\data\war\war_list.txt', 'a', encoding='utf-8') as file_obj:
        with open('D:\military\military\data\war\\raw_war_list.txt', 'r', encoding='utf-8') as raw_file_obj:
            raw_str = raw_file_obj.readline()
            while raw_str != "":
                list = raw_str.split('\t')
                if (len(list) == 6) :
